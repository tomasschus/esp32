...
import type { GraphHopperGeocodeResponse, GeocodedHit, GraphHopperHit } from "../types/geocode";
...
function buildLabel(hit: GraphHopperHit): string {
  const parts: string[] = [];
  if (hit.street) {
    parts.push(hit.housenumber ? `${hit.street} ${hit.housenumber}` : hit.street);
  } else {
    parts.push(hit.name);
  }
...
// GET /geocode?q=muñiz+721&lat=-38.222505&lon=-60.909195
router.get("/", async (req: Request, res: Response) => {
  const { q, lat, lon } = req.query as Record<string, string | undefined>;
  ...
  try {
    ...
    console.error("Error al consultar GraphHopper geocode:", err);
